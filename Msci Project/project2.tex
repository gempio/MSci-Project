\documentclass{report} 
  ...
  \title{Multi-robot to Human Communication}
  \author{Maciej Musialek}
  \maketitle

  \tableofcontents
  \chapter{What has been finished}
    I have already finished working on the Robot environment with single robot point on click. Next steps are:
      \begin{itemize}
        \item Start work on the GUI to allow one robot to be directed from outside the environment \textbf{This is done for two robots atm}
        \item Restructure the code to start spawning robots and allow the GUI to move different robots around the plane  \textbf{This is currently done for one and two robots. The problem is spawning is only at the start}
        \item Add energy levels so that the robot stops moving after its energy is depleted \textbf{Done partially. The robot will still move after energy depletion}
        \item Add picture taking simulation to allow treasure retrieval from specific rooms \textbf{Not done yet}
        \item Sync the GUI with the ROS environment that will provide constant updates to the robot.

        \item Add code that will allow robots to allow them to move by themselves and give them the power to take independent action rather than asking users.\textbf{Not done yet}
        \item Start testing and evaluation
      \end{itemize}

    Through this weekend I will work on the first two points on the agenda this weekend and try to get this to work by Monday. I have dedicated this whole weekend for that sole purpose. I will try to send progress report on Monday/Tuesday and by Friday try to add energy levels for moving robots.

    Adding picture simulation and robot independent action I will try to focus on the next weekend to make sure that I can finally start working on testing and writing down the evaluation.

  \chapter{Introduction}
    Communication has been a subject of research for a long time now. With the development of computers and robotics a new area was opened regarding communication between humans and robots. With the addition of job automation allowing robots to perform tasks on their own and take instructions, the communication started playing an even bigger part in job distribution. Humans handle the job distribution and robots would perform tasks carried onto them.(NOTE: Attach references here)

    A lot of research has and is still being carried out about how to handle the communication with majority of it focusing on turn-taking communication.(NOTE: Elaborate more here with papers)

    This project will also focus on turn-taking communication for energy limited robots with an addition of trying to give some decision power to robots, like proposing potential moves rather than list all of them. It focuses on creating a robot platform with a user interface that will allow testing how single human interaction can improve multi robot collaboration. This will be done by comparing different levels of freedom to robots from no control to complete control over the environment and their choices.(Is this not too brief? Tried to make it brief since its only the introduction.)

    A simple setting will be set up for a treasure hunt game and robots will be directed to various rooms with an intent to gather necessary treasures for the player at minimum possible expense of energy. Considerations will also be taken about how to device the most efficient GUI for multi-robot task allocation.(Again maybe too brief?)

  \chapter{Background \textbf{PLEASE FOCUS HERE}}
    ...
    \section{Related work in Human-Robot collaboration}
      ...
      \subsection{Same space collaboration}
        In their papers Chang Liu et al.\cite{ChangLiu} and Vijay Govindarajan et al.\cite{Vijay} discuss collaboration in same space. 
    ...
    \section{Research in ROS environment}
      ...
      \subsection{Conclusion on ROS research}
  ...
  \chapter{TESTING - Modes Specification and analysis (Finish this after next meeting with what is had)}
    ...
    \section{Known problems}
      ...
      \subsection{Environment mostly simulated}
      ...
      \subsection{System currently made to run in an uninterrupted environment}
      ...
      \subsection{A very strict Server}
  ...
  \begin{thebibliography}{9}
    ...
    \bibitem{Vijay}
      ...
      \emph{Distributed Autonomous Robotic Systems Volume 112 of the series Springer Tracts in Advanced Robotics pp 17-32}.
    ...
    \bibitem{Fiore}
      Michelangelo Fiore, Aurelie Clodic, Rachid Alami.On Planning and Task achievement Modalities for Human-Robot Collaboration. \emph{International Symposium on Experimental Robotics (ISER 2014), Jun 2014, Marrakech, Morocco. 15p. <hal-01149109>}.

    \bibitem{ChienLiang}
      Chien-Liang Fok et al. Integration and Usage of a ROS-Based Whole Body Control Software Framework
      \emph{Robot Operating System (ROS)Volume 625 of the series Studies in Computational Intelligence pp 535-563}.
